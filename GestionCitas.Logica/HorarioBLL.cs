    ...
    public class HorarioBLL
    {
        ...
        public Boolean Valida(HorarioDTO item)
        {
            ...
            if (item.HoraInicio != TimeSpan.Parse("00:00") && item.HoraFin != TimeSpan.Parse("00:00"))
            {
                ...
                if (item.ProfesionalId > 0 && item.FechaAtencion != DateTime.Parse("0001-01-01"))
                {
                    horarioProfesional = HorarioBLL.Instancia.ListarHorariosPorProfesional(item.ProfesionalId)
                                    .Where(
                                            x =>
        ...
        public List<HorarioDTO> ListarHorariosPorProfesional(Int32 profesionalId)
        {
            try
            {
                return HorarioDAL.Instancia.ListarHorariosPorProfesional(profesionalId);
            }
            catch (Exception e)
            {
