...
namespace GestionCitas.Logica
{
    public class CitaBLL
    {
        ...
        public Boolean Valida(CitaDTO item)
        {
            ...
            List<HorarioDTO> horarioProfesional = null;
            List<CitaDTO> citasProfesional = null;
            Boolean horarioDisponible = false;

            if (item.ClienteId <= 0)
                Mensaje += "Por favor, debe indicar el Cliente\n\r";
            if (item.ProfesionalId <= 0)
                Mensaje += "Por favor, debe indicar el Profesional\n\r";
            ...
            if (item.HoraInicio != TimeSpan.Parse("00:00") && item.HoraFin != TimeSpan.Parse("00:00"))
            {
                ...
                if (item.ProfesionalId > 0)
                {
                    horarioProfesional = HorarioBLL.Instancia.ListarHorariosPorProfesional(item.ProfesionalId).Where(x => x.Activo == true && x.FechaAtencion == item.FechaAtencion && (item.HoraInicio >= x.HoraInicio && item.HoraInicio < x.HoraFin) && (item.HoraFin > x.HoraInicio && item.HoraFin <= x.HoraFin)).ToList();

                    if (horarioProfesional == null || horarioProfesional.Count == 0)
                    ...
                    {
                        citasProfesional = CitaBLL.Instancia.ListarCitasProfesional(item.ProfesionalId, item.FechaAtencion).Where(x => x.Id != item.Id && x.Activo == true && ((item.HoraInicio >= x.HoraInicio && item.HoraInicio < x.HoraFin) || (item.HoraFin > x.HoraInicio && item.HoraFin <= x.HoraFin))).ToList();

                        if (citasProfesional != null && citasProfesional.Count > 0)
        ...
        public List<CitaDTO> ListarCitasProfesional(Int32 profesionalId, DateTime fechaAtencion)
        {
            try
            {
                return CitaDAL.Instancia.ListarCitasProfesional(profesionalId, fechaAtencion);
            }
            catch (Exception e)
            {
